//
//  PLVerticalCollectionFlowLayout.m
//  PomegranateLive
//
...
@implementation PLVerticalCollectionFlowLayout
...
- (NSArray *) layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    NSMutableArray *arrSuperLayoutAttributes = [[super layoutAttributesForElementsInRect:rect] mutableCopy];
    
    //创建存索引的数组，无符号（正整数），无序（不能通过下标取值），不可重复（重复的话会自动过滤）
    NSMutableIndexSet *noneHeaderSections = [NSMutableIndexSet indexSet];
    //遍历arrSuperLayoutAttributes，得到一个当前屏幕中所有的section数组
    for (UICollectionViewLayoutAttributes *attributes in arrSuperLayoutAttributes)
    {
    ...
    //遍历arrSuperLayoutAttributes，将当前屏幕中拥有的header的section从数组中移除，得到一个当前屏幕中没有header的section数组
    ...
    [noneHeaderSections enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *stop){
        ...
        if (attributes)
        {
            //将该header结构信息重新加入到arrSuperLayoutAttributes中去
            [arrSuperLayoutAttributes addObject:attributes];
        }
    }];
    
    //遍历arrSuperLayoutAttributes，改变header结构信息中的参数，使它可以在当前section还没完全离开屏幕的时候一直显示
    for (UICollectionViewLayoutAttributes *attributes in arrSuperLayoutAttributes) {
    ...
    //转换回不可变数组，并返回
    return [arrSuperLayoutAttributes copy];
